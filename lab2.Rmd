...
```{r}
folds <- 10 # breaking into 10 chunks and test is against stuff we sent aside
fold_vec <- rep(1:folds, length.out = nrow(penguins_clean))

set.seed(42)  # shuffling 
...
rmse_predict <- predict_test %>% 
  summarize(rmse_mdl1 = calc_rmse(model1, mass),
            ...
            rmse_mdl3 = calc_rmse(model3, mass)) # x= model 1 and y=mass
```
...
for (i in 1:folds) {
  ...
  kfold_rmse_df <- kfold_pred_df %>% 
    summarize(rmse_mdl1 = calc_rmse(mdl1, mass),
              ...
              test_gp = i)
  rmse_df <- bind_rows(rmse_df, kfold_rmse_df) # "." shorthand to put in something you keep typing
}
